...
def analyze_accounts(accounts: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    id_key = resolved.get("id")
    hierarchy: dict[str, Any] = {}
    if parent_key and id_key:
        parents = [account.get(parent_key) for account in accounts]
        roots = [account for account in accounts if account.get(parent_key) in (None, "", 0)]
        children_per_parent = Counter(p for p in parents if p not in (None, "", 0))
        by_id = {account.get(id_key): account for account in accounts}
        depths = []
        for account in accounts:
            depth, cursor, guard = 0, account, 0
            while cursor is not None and cursor.get(parent_key) not in (None, "", 0) and guard < 20:
        ...
        hierarchy = {
            ...
            "root_accounts": [
                {"id": a.get(id_key), "name": a.get(resolved.get("name") or "name")} for a in roots[:20]
            ],
    ...
    for field in ("chow_current_account", "duplicate_of_account"):
        ...
        if key:
            ...
            link_fields[field] = {
                ...
                "types": dict(Counter(_type_name(v) for v in values)),
                "samples": [v for v in values if v is not None][:SAMPLE_LIMIT],
            }
    ...
    if note_key:
        ...
        note_summary = {
            ...
            "non_null": sum(1 for n in notes if n),
    ...
    return {
        ...
        "always_populated": [name for name in field_names if non_null_counts[name] == total],
        "sometimes_null": [name for name in field_names if 0 < non_null_counts[name] < total],
        "always_null": [name for name in field_names if non_null_counts[name] == 0],
...
def format_report(report: dict[str, Any], envelope: dict[str, Any], me: Any) -> str:
    ...
    add("")
    add("-- fields (name: non-null/total, types) --")
    for name, info in report["fields"].items():
        add(f"  {name}: {info['non_null']}/{report['account_count']}  {info['types']}")
    add("")
    add(f"-- always populated: {report['always_populated']}")
    add(f"-- sometimes null:   {report['sometimes_null']}")
    add(f"-- always null:      {report['always_null']}")
    add("")
    ...
    if report["hierarchy"]:
        ...
        for root in hierarchy["root_accounts"]:
            add(f"    id={root['id']!r} name={root['name']!r}")
    ...
    if report["link_fields"]:
        ...
        for field, info in report["link_fields"].items():
            add(f"  {field} (key {info['resolved_key']!r}): non-null={info['non_null']} types={info['types']}")
            if info["samples"]:
